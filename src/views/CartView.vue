...
const { items, total, removeItem, clearCart, buildMailto } = useCart()
    ...
    <template v-else>
      <div class="space-y-4 mb-8">
        <div
          v-for="item in items"
          :key="item.key"
          class="flex items-center gap-4 bg-cream-dark rounded-lg p-3"
        >
          <img
      ...
      <div class="border-t border-plum/10 pt-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <button
            class="px-4 py-2 text-sm text-plum-muted hover:text-red-600 transition-colors"
            @click="clearCart"
          >
            Vider le panier
          </button>
