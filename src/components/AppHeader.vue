...
<template>
  <header class="sticky top-0 z-50 bg-white shadow-sm">
    <nav class="max-w-6xl mx-auto px-4 md:px-6 py-4 md:py-8 flex items-center justify-between">
      <!-- Burger mobile -->
      <button class="md:hidden text-plum-dark" @click="menuOpen = !menuOpen">
    ...
    <transition name="slide">
      <div v-if="menuOpen" class="absolute left-0 right-0 top-full md:hidden border-t border-plum/10 bg-white px-4 pb-4 shadow-md">
        <RouterLink
          v-for="link in [
            { to: '/', label: 'Accueil' },
            { to: '/galerie', label: 'Galerie' },
